...
- **Database**: MongoDB & Mongoose.
  - _Optimization_: Leverages `.lean()` queries to bypass model hydration and custom connection pooling.
  - _Database Indexes_: Optimized compound index on `{userId, date}` for fast expense filtering and unique compound index on `{userId, category}` for budgets.
